...
class StorageService
{
    ...
    /**
     * 仅在实际使用百度云 BOS 时加载 SDK，避免其他存储方式也被 SDK 相对路径影响。
     *
     * @return void
     */
    private static function loadBaiduBosSdk()
    {
        if (class_exists(BosClient::class, false)) {
            return;
        }

        $sdkPath = dirname(__DIR__, 4) . DIRECTORY_SEPARATOR . 'extend' . DIRECTORY_SEPARATOR . 'bce-php-sdk-0.9.21' . DIRECTORY_SEPARATOR . 'BaiduBce.phar';
        if (!is_file($sdkPath)) {
            exception('百度云 BOS SDK 文件不存在：' . $sdkPath);
        }

        require_once $sdkPath;

        if (!class_exists(BosClient::class, false)) {
            exception('百度云 BOS SDK 加载失败');
        }
    }
    ...
    public static function upload($file_info, $pk = 'file_id')
    {
        $errmsg = '';
        $file_info['domain'] = '';
        $file_name = ltrim($file_info['file_path'], '/');
        $file_path = dirname(__DIR__, 4) . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . str_replace(['/', '\\'], DIRECTORY_SEPARATOR, $file_name);
        $setting = SettingService::info();
        $storage = $setting['storage'];
        if ($storage !== 'local' && self::isLocalRuntime()) {
