...
use app\common\cache\inspection\InspectionSettingCache;
use app\common\model\inspection\InspectionSettingModel;
 ...
 * 系统设置
 */
class InspectionSettingService
{
    /**
     * 设置id
     * @var integer
    ...
    public static function info($fields = '')
    {
        ...
        $info = InspectionSettingCache::get($key);
        if (empty($info)) {
            $model = new InspectionSettingModel();
            $pk = $model->getPk();
            ...
            $info['token_type'] = Config::get('inspection.token_type');
            $info['token_name'] = Config::get('inspection.token_name');
            $info['token_exps'] = $info['token_exp'] * 3600;

            InspectionSettingCache::set($key, $info);
        }
