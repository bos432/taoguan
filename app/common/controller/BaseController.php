...
abstract class BaseController
{
     ...
     * @return array
     */
    protected function buildWhere(array $fields = [])
    {
        $where = [];
        ...
        foreach ($fields as $field) {
            ...
            if (strpos($field, '|')) {
                ...
                $value = $request->param($fields[0]);
            }else{
                $value = $request->param($field);
            }
            if (isset($value) && $value !== '' && $value!=-1 && $value !== 'null' && $value !== '[]') {
    ...
    protected function order($default = [])
    {
        ...
        if ($sort_field && $sort_value && $sort_field!=='null'  && $sort_value!=='null') {
    ...
    protected function params($fields = [], $filter = '')
    {
        ...
        foreach ($fields as $key => $val) {
            ...
            if ($name) {
                ...
                $value = $this->request->param($name . '/' . $type, $default, $filter);
