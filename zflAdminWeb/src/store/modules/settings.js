...
export const useSettingsStore = defineStore('setting', () => {
  ...
  function normalizeStoredText(settingRef, fallbackValue) {
    const currentValue = String(settingRef.value || '').trim()
    if (!currentValue) {
      settingRef.value = fallbackValue
      return
    }
    if (legacyNameMap.has(currentValue)) {
      settingRef.value = legacyNameMap.get(currentValue)
    }
  }

  normalizeStoredText(systemName, defaultSettings.systemName)
  normalizeStoredText(pageTitle, defaultSettings.pageTitle)
