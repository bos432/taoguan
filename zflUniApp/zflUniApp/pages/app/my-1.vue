<template>
	<view class="zaiui-my-box" :class="[show ? 'show' : '', 'ui-theme-' + uiThemeStyle]" :style="myThemeCssVars">
...
<!--					<text class="cuIcon-camera"/>-->
			...
			<view class="zaiui-user-info-box">
				<view v-if="reviewMode" class="review-user-view">
					...
					<button class="cu-btn sm round review-service-btn" @tap="jump('/pages/help/service')">联系客服</button>
				</view>
				<!--未登陆-->
          ...
          <!-- #ifndef MP-WEIXIN -->
          <view class="text-black text-lg text-bold padding-sm my-head-login-text" @tap="loginUrlTap">点击登录
          </view>
          <!-- #endif -->
          <!-- 授权登录按钮 -->
          <!-- #ifdef MP-WEIXIN -->
            ...
            <view class="cu-avatar round lg" :style="displayAvatarStyle" />
						<view class="content text-xl">
							...
							<view class="text-white-bg text-sm">
								<text class="text-border-x" v-if="userInfo.is_balance">司秤员</text>
                <text class="text-border-x" v-if="userInfo.is_warehouse">仓库员</text>
                <text class="text-border-x" v-if="userInfo.group_names">{{userInfo.group_names}}</text>
                <text class="text-border-x" v-if="userInfo.tag_names">{{userInfo.tag_names}}</text>
							</view>
						</view>
					</view>
				</view>
          ...
          <view class="cu-item" @tap="setupTap('/pages/order/list?status=4')">
			...
			<!--用户提示-->
			<view class="text-sm zaiui-user-info-tip-box" v-if="!login && !reviewMode"  @tap="setupTap('/pages/merchant/apply')">
        ...
        <view v-if="!reviewMode && recentActionSummary" class="runtime-action-card">
          <view class="runtime-action-card__title">最近操作</view>
          <view class="runtime-action-card__desc">{{ recentActionSummary }}</view>
        </view>
        <view v-if="!reviewMode && recentReleasedGoodsCard" class="recent-release-panel" @tap="openRecentReleasedGoods">
          <view class="recent-release-panel__badge">最近发布</view>
          <view class="recent-release-panel__title">{{ recentReleasedGoodsCard.title || '新商品' }}</view>
          <view class="recent-release-panel__desc">已保留发布后回看入口，可直接返回商城继续核对商品详情、价格与图片。</view>
          <view class="recent-release-panel__action">
            <text>前往查看</text>
            <text class="cuIcon-right"></text>
          </view>
        </view>
        <view v-if="!reviewMode && primaryWorkbenchEntry" class="workbench-entry-panel" @tap="openPrimaryWorkbenchEntry">
          <view class="workbench-entry-panel__badge">工作台入口</view>
          <view class="workbench-entry-panel__title">{{ workbenchEntryTitle }}</view>
          <view class="workbench-entry-panel__desc">{{ workbenchEntryDesc }}</view>
          <view class="workbench-entry-panel__action">
			...
			<!--用户数据-->
			<view class="padding-xs bg-white zaiui-user-info-order-box" v-if="!reviewMode">
          ...
          >
			...
			<view v-if="showPendingWriteoffCard" class="pending-reminder-card" @tap="openPendingWriteoff">
				<view class="pending-badge">订单提醒</view>
				<view class="pending-title">您有 {{ pendingWriteoffCountText }} 笔待核销订单</view>
				<view class="pending-desc">
					待核销金额 ¥{{ pendingWriteoffAmountText }}<text v-if="pendingWriteoffOldestTime">，最早一笔：{{ pendingWriteoffOldestTime }}</text>
				</view>
			...
			<view class="padding-xs bg-white margin-top zaiui-user-info-tools-box">
				<view class="padding-sm tools-view">
					<view class="text-black text-bold text-lg tools-title">{{ toolsHeaderTitle }}</view>
        ...
        >
          <view class="merchant-identity-picker__inner">
            ...
            <text class="merchant-identity-picker__value">{{ (currentMerchantIdentity && currentMerchantIdentity.identity_name) || '请选择商家身份' }}</text>
          </view>
        </picker>

				<view v-if="showMerchantWorkbenchSummary" class="merchant-tools-summary">
					<view class="merchant-tools-summary__head">
						<view>
							<view class="merchant-tools-summary__badge">商家工作台</view>
							<view class="merchant-tools-summary__title">{{ merchantWorkbenchSummaryTitle }}</view>
						</view>
						<view class="merchant-tools-summary__status" :class="merchantStatusTone">{{ merchantStatusText }}</view>
					</view>
					<view class="merchant-tools-summary__desc">{{ merchantWorkbenchSummaryDesc }}</view>
          <picker
            v-if="false"
              ...
              <text class="merchant-identity-picker__value">{{ (currentMerchantIdentity && currentMerchantIdentity.identity_name) || '请选择商家身份' }}</text>
				...
				<view class="zaiui-tools-list-box">
					<view class="recommend-tools-grid" :class="recommendedToolsGridClass">
						<view v-for="item in recommendedTools" :key="item.code" class="tool-grid-cell">
							<view class="cu-item tool-grid-card" :class="item.cardClass" @tap="gridTap(item)">
                  ...
                  <text>{{ item.displayName }}</text>
								...
								<text v-if="item.badge" class="cu-tag badge bg-red tool-badge">{{ item.badge }}</text>
							</view>
						</view>
					</view>
				</view>
        ...
        <view v-if="showFrontendSuperWorkbench" class="merchant-super-tools">
          <view class="merchant-super-tools__hero">
            ...
            <view class="merchant-super-tools__hero-title">两个高权限入口已就位</view>
            <view class="merchant-super-tools__hero-desc">平台商家审核、商家订单核销仅对商家超管开放，商品发布已下沉到普通商家入口。</view>
          </view>
          <view class="merchant-super-tools__grid">
            ...
            >
              ...
              <view class="merchant-super-tool-card__title">{{ item.displayName }}</view>
              <view class="merchant-super-tool-card__desc">{{ item.caption }}</view>
...
<script>
  ...
  import api from '@/api';
  import { resolveAdminNextUrl, resolveImageUrl as resolveSafeImageUrl } from '@/utils/resource.js';
  ...
  import { buildEnvConfirmText, getEnvIsolationTag } from '@/utils/env-risk.js';
  import {
  ...
  import { applyUiThemeFromSetting, resolveUiThemeStyle } from '@/utils/ui-theme.js';
...
function resolveReviewInfo(setting = {}) {
  ...
  return {
    ...
    review_hero_desc: system.review_hero_desc || '当前页面展示平台服务入口、官方资讯与联系渠道，可继续查看平台服务内容。'
  };
}
...
const MY_HEAD_THEME_PRESETS = {
  origin: {
    ...
    headGlowTwo: 'radial-gradient(circle, rgba(255,204,188,0.55) 0%, rgba(255,255,255,0.04) 72%, rgba(255,255,255,0) 100%)',
    glassBg: 'linear-gradient(180deg, rgba(255, 255, 255, 0.2) 0%, rgba(255, 255, 255, 0.12) 100%)',
    ...
    tipBg: 'linear-gradient(135deg, rgba(255,255,255,0.2) 0%, rgba(255,255,255,0.12) 100%)',
  ...
  red_energy: {
    ...
    headGlowTwo: 'radial-gradient(circle, rgba(255,223,190,0.42) 0%, rgba(255,255,255,0.04) 72%, rgba(255,255,255,0) 100%)',
    glassBg: 'linear-gradient(180deg, rgba(255, 247, 242, 0.18) 0%, rgba(255, 236, 229, 0.12) 100%)',
    ...
    tipBg: 'linear-gradient(135deg, rgba(255, 247, 242, 0.18) 0%, rgba(255, 236, 229, 0.12) 100%)',
  ...
  yellow_energy: {
    ...
    headGlowTwo: 'radial-gradient(circle, rgba(255,236,170,0.58) 0%, rgba(255,255,255,0.04) 72%, rgba(255,255,255,0) 100%)',
    glassBg: 'linear-gradient(180deg, rgba(255, 255, 255, 0.34) 0%, rgba(255, 247, 221, 0.26) 100%)',
    ...
    tipBg: 'linear-gradient(135deg, rgba(247, 208, 86, 0.96) 0%, rgba(255, 234, 155, 0.96) 58%, rgba(255, 248, 226, 0.96) 100%)',
  ...
  jade_modern: {
    ...
    headGlowTwo: 'radial-gradient(circle, rgba(229,204,143,0.42) 0%, rgba(255,255,255,0.04) 72%, rgba(255,255,255,0) 100%)',
    glassBg: 'linear-gradient(180deg, rgba(255, 255, 255, 0.16) 0%, rgba(247, 244, 236, 0.1) 100%)',
    ...
    tipBg: 'linear-gradient(135deg, rgba(255, 255, 255, 0.16) 0%, rgba(247, 244, 236, 0.12) 100%)',
...
const MY_PAGE_THEME_PRESETS = {
  origin: {
    toolsPanelBg: 'linear-gradient(180deg, rgba(255, 252, 246, 0.99) 0%, rgba(250, 245, 237, 0.98) 100%)',
    toolsPanelShadow: '0 18rpx 38rpx rgba(118, 88, 37, 0.08)',
    toolsSubtleText: '#9b8165',
    summaryBg: 'linear-gradient(135deg, rgba(252, 242, 224, 0.92) 0%, rgba(250, 232, 211, 0.96) 100%)',
    ...
    toolIconBg: 'linear-gradient(135deg, rgba(244, 195, 92, 0.34) 0%, rgba(232, 151, 49, 0.26) 100%)',
    ...
    superHeroBg: 'linear-gradient(180deg, rgba(255, 250, 242, 0.98) 0%, rgba(255, 247, 235, 0.98) 100%)',
    superHeroBorder: 'rgba(238, 191, 105, 0.24)',
    superBadgeBg: 'rgba(243, 189, 92, 0.18)',
    superBadgeColor: '#c7851e',
    superTitle: '#7a5330',
    superDesc: '#96795b',
    superCardBg: 'linear-gradient(180deg, rgba(255, 255, 255, 0.99) 0%, rgba(255, 249, 240, 0.98) 100%)',
    ...
    superAction: '#d18a24',
    superPrimaryBg: 'linear-gradient(180deg, rgba(255, 238, 221, 0.99) 0%, rgba(255, 227, 199, 0.98) 100%)',
    ...
    superSecondaryBg: 'linear-gradient(180deg, rgba(255, 251, 244, 0.99) 0%, rgba(255, 246, 233, 0.98) 100%)',
    ...
    superAlertBg: 'linear-gradient(180deg, rgba(255, 252, 245, 0.99) 0%, rgba(255, 246, 230, 0.98) 100%)',
  ...
  red_energy: {
    toolsPanelBg: 'linear-gradient(180deg, rgba(255, 246, 244, 0.99) 0%, rgba(252, 238, 234, 0.98) 100%)',
    toolsPanelShadow: '0 18rpx 38rpx rgba(156, 46, 42, 0.1)',
    toolsSubtleText: '#a76865',
    summaryBg: 'linear-gradient(135deg, rgba(255, 236, 232, 0.96) 0%, rgba(255, 224, 219, 0.98) 100%)',
    ...
    toolIconBg: 'linear-gradient(135deg, rgba(223, 78, 80, 0.2) 0%, rgba(240, 133, 102, 0.24) 100%)',
    ...
    superHeroBg: 'linear-gradient(180deg, rgba(255, 245, 242, 0.99) 0%, rgba(255, 236, 232, 0.99) 100%)',
    superHeroBorder: 'rgba(214, 101, 87, 0.22)',
    superBadgeBg: 'rgba(163, 12, 21, 0.1)',
    superBadgeColor: '#a30c15',
    superTitle: '#7a2525',
    superDesc: '#946060',
    superCardBg: 'linear-gradient(180deg, rgba(255, 252, 251, 0.99) 0%, rgba(255, 242, 238, 0.98) 100%)',
    ...
    superAction: '#c44036',
    superPrimaryBg: 'linear-gradient(180deg, rgba(255, 234, 228, 0.99) 0%, rgba(255, 218, 209, 0.98) 100%)',
    ...
    superSecondaryBg: 'linear-gradient(180deg, rgba(255, 249, 246, 0.99) 0%, rgba(255, 240, 234, 0.98) 100%)',
    ...
    superAlertBg: 'linear-gradient(180deg, rgba(255, 246, 242, 0.99) 0%, rgba(255, 232, 225, 0.98) 100%)',
  ...
  jade_modern: {
    toolsPanelBg: 'linear-gradient(180deg, rgba(246, 243, 236, 0.99) 0%, rgba(240, 236, 226, 0.98) 100%)',
    toolsPanelShadow: '0 18rpx 38rpx rgba(33, 82, 73, 0.09)',
    toolsSubtleText: '#6f7367',
    summaryBg: 'linear-gradient(135deg, rgba(232, 241, 236, 0.96) 0%, rgba(225, 236, 229, 0.98) 100%)',
    ...
    toolIconBg: 'linear-gradient(135deg, rgba(47, 123, 110, 0.18) 0%, rgba(109, 155, 126, 0.24) 100%)',
    ...
    superHeroBg: 'linear-gradient(180deg, rgba(241, 243, 236, 0.99) 0%, rgba(232, 236, 227, 0.99) 100%)',
    superHeroBorder: 'rgba(74, 114, 96, 0.18)',
    superBadgeBg: 'rgba(27, 111, 99, 0.1)',
    superBadgeColor: '#1b6f63',
    superTitle: '#22463f',
    superDesc: '#65726a',
    superCardBg: 'linear-gradient(180deg, rgba(252, 250, 246, 0.99) 0%, rgba(243, 239, 231, 0.98) 100%)',
    ...
    superAction: '#1b6f63',
    superPrimaryBg: 'linear-gradient(180deg, rgba(232, 244, 238, 0.99) 0%, rgba(221, 236, 228, 0.98) 100%)',
    ...
    superSecondaryBg: 'linear-gradient(180deg, rgba(248, 247, 242, 0.99) 0%, rgba(238, 234, 225, 0.98) 100%)',
    ...
    superAlertBg: 'linear-gradient(180deg, rgba(244, 247, 239, 0.99) 0%, rgba(235, 241, 231, 0.98) 100%)',
    ...
    computed: {
      ...
      myRuntimeStatusText() {
        return this.recentReleasedGoodsCard ? '存在最近发布回看入口' : '暂无最近发布记录';
      },
      myEntryUsageText() {
      ...
      primaryWorkbenchEntry() {
        const superTools = Array.isArray(this.superManageTools) ? this.superManageTools : [];
        if (superTools.length > 0) {
          return superTools[0];
        }
        const tools = Array.isArray(this.recommendedTools) ? this.recommendedTools : [];
      ...
      safeStatusNums() {
        const source = this.userInfo && this.userInfo.status_nums && typeof this.userInfo.status_nums === 'object'
      ...
      guestAvatarUrl() {
        return this.resolveSafeAvatarUrl(this.default_avatar_url, this.avatarPool[0] || '/static/images/avatar/1.jpg');
      },
      guestAvatarStyle() {
      ...
      displayAvatarUrl() {
        ...
        if (this.hasCustomAvatar && avatarUrl) {
          return this.resolveSafeAvatarUrl(avatarUrl, this.pickAvatarBySeed(this.getUserSeed()));
        }
        return this.resolveSafeAvatarUrl(this.pickAvatarBySeed(this.getUserSeed()), '/static/images/avatar/1.jpg');
      },
      displayAvatarStyle() {
      ...
      displayNickname() {
        ...
        const mobile = this.normalizeText(this.userInfo.mobile) || this.normalizeText(this.userInfo.phone);
        if (mobile) {
      ...
      currentMerchantIdentity() {
        ...
        const list = Array.isArray(this.merchantIdentityList) ? this.merchantIdentityList : [];
        return list.find(item => Number(item.mer_user_id || 0) === currentId) || list[0] || null;
      },
      currentMerchantIdentityIndex() {
        const currentId = Number(this.currentMerchantIdentityId || 0);
        const list = Array.isArray(this.merchantIdentityList) ? this.merchantIdentityList : [];
        const index = list.findIndex(item => Number(item.mer_user_id || 0) === currentId);
        return index >= 0 ? index : 0;
      },
      merchantIdentityOptions() {
        return (Array.isArray(this.merchantIdentityList) ? this.merchantIdentityList : []).map(item => ({
          mer_user_id: Number(item.mer_user_id || 0),
          merchant_id: Number(item.merchant_id || 0),
          label: item.identity_name || (item.merchant && item.merchant.title) || '商家身份'
        }));
      },
      merchantPermissionMap() {
        const map = {};
        (Array.isArray(this.merchantPermissionCodes) ? this.merchantPermissionCodes : []).forEach(code => {
          if (code) {
            map[String(code)] = 1;
          }
        });
      ...
      reviewDisplayTitle() {
        return this.normalizeText(this.reviewInfo.system_name)
          || this.normalizeText(this.reviewInfo.review_hero_title)
          || '涛冠优选';
      },
      reviewDisplayDesc() {
        return this.normalizeText(this.reviewInfo.review_hero_desc)
          || '当前页面展示平台服务入口、官方资讯与联系渠道，可继续查看平台服务内容。';
      },
      hasMerchantRecord() {
        return Number(this.merchantInfo.id || 0) > 0 || Number(this.userInfo.is_merchant || 0) === 1 || !!this.currentMerchantIdentity;
      },
      ...
      isMerchantSuper() {
        return Number((this.currentMerchantIdentity && this.currentMerchantIdentity.merchant_user && this.currentMerchantIdentity.merchant_user.is_super) || 0) === 1;
      },
      releaseAllowed() {
        return this.hasMerchantRecord
      ...
      merchantStatusText() {
        ...
        if (!Number(this.merchantInfo.id || 0) && Number(this.userInfo.is_merchant || 0) === 1) {
          return '审核通过';
        }
        ...
        return this.normalizeText(this.merchantInfo.expire_status_title) || '正常有效';
      },
      merchantStatusTone() {
        if (!this.hasMerchantRecord) {
          return 'is-idle';
        ...
        if (Number(this.merchantInfo.auth_state || 0) === 2 || Number(this.merchantInfo.is_expired || 0) === 1) {
          return 'is-danger';
        }
        if (Number(this.merchantInfo.auth_state || 0) === 0 || Number(this.merchantInfo.should_remind || 0) === 1) {
          return 'is-warning';
        }
        return 'is-safe';
      },
      ...
      merchantServiceDesc() {
        ...
        if (Number(this.merchantInfo.auth_state || 0) === 2) {
          return this.normalizeText(this.merchantInfo.auth_msg) || '审核未通过，请根据原因完善资料后重新提交。';
        }
        if (Number(this.merchantInfo.is_expired || 0) === 1) {
      ...
      merchantServiceActions() {
        ...
        if (this.merchantAuthApproved) {
          ...
          if (this.pendingWriteoffCount > 0 && Number(this.merchantInfo.is_expired || 0) !== 1) {
            actions.push({
      ...
      showMerchantWorkbenchSummary() {
        return !this.reviewMode && !this.login && this.hasMerchantRecord && !this.showFrontendSuperWorkbench;
      },
      ...
      recommendedTools() {
        const order = this.reviewMode
          ...
          : ['service', 'news', 'friend', 'read', 'analytics', 'renew', 'settings', 'release'];
        return order
      ...
      superManageTools() {
        if (this.reviewMode || this.login || !this.isMerchantSuper) {
          return [];
        }
      ...
      showFrontendSuperWorkbench() {
        return this.superManageTools.length > 0;
      },
      toolsHeaderTitle() {
      ...
      myThemeCssVars() {
        const palette = MY_PAGE_THEME_PRESETS[this.uiThemeStyle] || MY_PAGE_THEME_PRESETS.origin;
        const headPalette = MY_HEAD_THEME_PRESETS[this.uiThemeStyle] || MY_HEAD_THEME_PRESETS.origin;
        return buildCssVars({
      ...
      pendingWriteoffCountText() {
        return this.pendingWriteoffCount > 99 ? '99+' : String(this.pendingWriteoffCount || 0);
      },
      ...
      showPendingWriteoffCard() {
        return !this.reviewMode
          ...
          && Number(this.merchantInfo.is_expired || 0) !== 1
          && this.hasMerchantPermission('verify_order', 'verify_cross_merchant_order')
    ...
    onShow() {
      this.setRecentActionSummary('刷新我的页面状态', '准备同步订单、身份和工作台入口');
      ...
      openPrimaryWorkbenchEntry() {
        ...
        this.setRecentActionSummary('打开主工作台入口', entry.displayName || entry.name || '工作台');
        this.gridTap(entry);
      },
      hasCachedAuth() {
      ...
      syncDefaultAvatar() {
        if (this.$store.state.setting && this.$store.state.setting.member) {
          if (this.$store.state.setting.member.default_avatar_url) {
            this.default_avatar_url = this.$store.state.setting.member.default_avatar_url;
          }
      ...
      loginUrlTap() {
        ...
        setLoginRedirect('/pages/app/my-1');
        uni.navigateTo({
          url: buildLoginRedirectUrl('/pages/app/my-1')
        });
      },
      getMiniappLoginCode() {
      ...
      onGetPhoneNumber(e) {
        ...
        this.getMiniappLoginCode()
          .then((loginCode) => api.miniappLogin({
            code: loginCode,
      ...
      ensureMemberInfoLoaded(force = false) {
        ...
        return api.getMemberInfo({}).then(res => {
          ...
          return this.loadMerchantIdentityState(force).then((list) => {
            if ((Array.isArray(list) && list.length > 0) || Number(res.data && res.data.is_merchant) === 1) {
      ...
      getMerchantInfo(force = false) {
        ...
        return api.merchantInfo({})
          .then(res => {
            this.merchantInfo = res.data || {};
            this.merchantPermissionCodes = this.resolveMerchantPermissionCodes(this.merchantInfo);
            if (Number(this.merchantInfo.current_mer_user_id || 0) > 0) {
              this.currentMerchantIdentityId = Number(this.merchantInfo.current_mer_user_id || 0);
      ...
      resolveMerchantPermissionCodes(merchantInfo = {}) {
        ...
        const isApprovedMerchant = (
          (Number(merchantInfo.id || 0) > 0 && Number(merchantInfo.auth_state || 0) === 1)
          || (!Number(merchantInfo.id || 0) && Number(this.userInfo.is_merchant || 0) === 1)
        );
        ...
        if (Array.isArray(merchantInfo.frontend_permission_codes) && merchantInfo.frontend_permission_codes.length > 0) {
          permissions.push.apply(permissions, merchantInfo.frontend_permission_codes);
        }
      ...
      handleMerchantExpireReminder() {
        ...
        const reminderKey = this.getMerchantReminderCacheKey(merchant, isExpired ? 'expired' : 'remind');
      ...
      handlePendingWriteoffReminder() {
        const merchant = this.merchantInfo || {};
        if (this.reviewMode || !merchant.id || Number(merchant.is_expired || 0) === 1 || this.pendingWriteoffCount <= 0) {
          return;
        }
        const reminderKey = this.getMerchantReminderCacheKey(merchant, `writeoff_${this.pendingWriteoffCount}`);
        if (cache.get(reminderKey)) {
          return;
        }
      ...
      loadMerchantIdentityState(force = false) {
        ...
        return Promise.all([
          ...
          api.merchantIdentityCurrent({}).catch(() => ({ data: { identity: {}, permissions: {} } })),
        ]).then(([listRes, currentRes]) => {
          const list = Array.isArray(listRes.data && listRes.data.list) ? listRes.data.list : [];
          this.merchantIdentityList = list;
          setMerchantIdentityList(list);
          const currentIdentity = currentRes.data && currentRes.data.identity ? currentRes.data.identity : {};
          const permissions = currentRes.data && currentRes.data.permissions ? currentRes.data.permissions : {};
          let currentId = Number(currentIdentity.mer_user_id || getCurrentMerchantIdentityId() || 0);
          if (!currentId && list.length) {
            currentId = Number(list[0].mer_user_id || 0);
          }
          this.currentMerchantIdentityId = currentId;
          setCurrentMerchantIdentityId(currentId);
          this.merchantPermissionCodes = Array.isArray(permissions.permission_codes) ? permissions.permission_codes : [];
          return list;
        }).finally(() => {
      ...
      onMerchantIdentityChange(event) {
        ...
        if (!nextItem || !nextItem.mer_user_id || Number(nextItem.mer_user_id) === Number(this.currentMerchantIdentityId || 0)) {
          return;
        }
        this.setRecentActionSummary('切换商家身份', nextItem.label || '已切换');
        ...
        uni.showModal({
          ...
          success: ({ confirm }) => {
            ...
            api.merchantIdentitySwitch({
              mer_user_id: Number(nextItem.mer_user_id)
            }).then(res => {
              const identity = res.data && res.data.identity ? res.data.identity : {};
              const permissions = res.data && res.data.permissions ? res.data.permissions : {};
              this.currentMerchantIdentityId = Number(identity.mer_user_id || nextItem.mer_user_id || 0);
              setCurrentMerchantIdentityId(this.currentMerchantIdentityId);
              this.merchantPermissionCodes = Array.isArray(permissions.permission_codes) ? permissions.permission_codes : [];
      ...
      findToolByCode(code) {
        return (Array.isArray(this.toolsList) ? this.toolsList : []).find(item => item.code === code) || null;
      },
      getToolDisplayName(item) {
        ...
        return (item && item.displayName) || labels[code] || (item && item.name) || '';
      },
      ...
      getSuperToolBadge(item) {
        ...
        if ((code === 'cross_writeoff' || code === 'order_audit') && this.pendingWriteoffCount > 0) {
      ...
      getToolCardClass(item) {
        ...
        return {
          ...
          'is-system': ['service', 'news', 'settings', 'accord_center'].includes(code),
          'is-admin': ['merchant_audit', 'order_audit', 'cross_writeoff'].includes(code),
        };
      },
      ...
      hasAdminPermission() {
        const keys = Array.prototype.slice.call(arguments);
        const permissions = this.userInfo && this.userInfo.admin_permissions ? this.userInfo.admin_permissions : {};
        for (let i = 0; i < keys.length; i++) {
      ...
      getUserSeed() {
        const source = this.normalizeText(this.userInfo.member_id)
      ...
      pickAvatarBySeed(seed) {
        const list = this.avatarPool && this.avatarPool.length ? this.avatarPool : [this.default_avatar_url];
        return list[seed % list.length] || this.default_avatar_url;
      },
      buildAvatarStyle(url, isGuest = false) {
        const safeUrl = this.resolveSafeAvatarUrl(url, '/static/images/avatar/1.jpg');
        const style = {
      ...
      resolveSafeAvatarUrl(url, fallback = '/static/images/avatar/1.jpg') {
        const safeFallback = this.normalizeText(fallback) || '/static/images/avatar/1.jpg';
      ...
      showTool(item) {
        if (this.reviewMode) {
          return item.code === 'service' || item.code === 'news' || item.code === 'accord_center';
        }
        if (item.code === 'friend') {
          return this.userInfo.wx_approved != 1 && (!this.hasMerchantRecord || this.hasMerchantPermission('edit_profile'));
        }
        if (item.code === 'renew') {
          return this.hasMerchantRecord && this.userInfo.wx_approved != 1 && this.hasMerchantPermission('admin_manage_merchant');
        }
        if (item.code === 'merchant_audit') {
          return this.hasMerchantRecord && this.isMerchantSuper && this.userInfo.wx_approved != 1;
        }
        if (item.code === 'order_audit') {
          return this.hasMerchantRecord && this.isMerchantSuper && this.userInfo.wx_approved != 1;
        }
        if (item.code === 'release' || item.code === 'read' || item.code === 'analytics') {
          return this.hasMerchantRecord
            && this.userInfo.wx_approved != 1
            && this.merchantAuthApproved
            && (
              (item.code === 'release' && this.hasMerchantPermission('publish_product'))
              || (item.code === 'read' && this.hasMerchantPermission('verify_order', 'verify_cross_merchant_order'))
              || (item.code === 'analytics' && this.hasMerchantPermission('view_stats'))
            );
        }
        return true;
      },
      getToolBadge(item) {
        if (item.code === 'read' && !this.reviewMode && this.pendingWriteoffCount > 0) {
          return this.pendingWriteoffCountText;
        }
        if (item.code === 'renew' && Number(this.merchantInfo.is_expired || 0) === 1) {
          return '到期';
        }
        if (item.code === 'renew' && Number(this.merchantInfo.should_remind || 0) === 1) {
          return '提醒';
        }
        return '';
      },
      handleMerchantServiceAction(item) {
        ...
        if (item.code === 'merchant_release' || item.code === 'merchant_analytics' || item.code === 'merchant_writeoff') {
      ...
      openPendingWriteoff() {
        if (!this.hasMerchantPermission('verify_order', 'verify_cross_merchant_order')) {
          this.setRecentActionSummary('打开待核销订单被拦截', '当前账号暂无核销权限');
          uni.showToast({
            icon: 'none',
            title: '暂无核销权限'
          });
        ...
        this.setRecentActionSummary('打开待核销订单', `待处理 ${this.pendingWriteoffCountText} 笔`);
        this.jump("/pages/order/mer_list");
      },
      openAdminNext() {
        const adminUrl = resolveAdminNextUrl(this.userInfo.admin_next_host);
        this.setRecentActionSummary('打开管理后台入口', adminUrl);
        // #ifdef H5
      ...
      ensureMerchantWorkbenchAvailable(requireSuper = false) {
        if (!this.hasMerchantRecord) {
          this.setRecentActionSummary('商家工作台校验失败', '当前账号尚未完成商家入驻');
          uni.showModal({
            title: '提示',
        ...
        if (requireSuper && !this.goodsReleaseEnabled) {
          this.setRecentActionSummary('商家工作台校验失败', '商品发布功能暂未开放');
          uni.showToast({
            icon: 'none',
            title: '商品发布功能暂未开放'
          });
        ...
        if (requireSuper && !this.isMerchantSuper) {
          this.setRecentActionSummary('商家工作台校验失败', '当前账号不是商家超管');
          uni.showToast({
            icon: 'none',
            title: '仅商家超管可用'
          });
        ...
        if (Number(this.merchantInfo.auth_state || 0) === 0) {
          this.setRecentActionSummary('商家工作台校验失败', '商家资料仍在审核中');
          uni.showToast({
            icon: 'none',
            title: '商家资料审核中'
          });
        ...
        if (Number(this.merchantInfo.auth_state || 0) === 2) {
          this.setRecentActionSummary('商家工作台校验失败', '商家资料审核未通过');
          uni.showModal({
            title: '审核未通过',
            content: this.normalizeText(this.merchantInfo.auth_msg) || '请完善商家资料后重新提交审核。',
            confirmText: '去完善',
            success: res => {
              if (res.confirm) {
                this.jump('/pages/merchant/apply');
      ...
      ensureMerchantSuperAccess(featureName = '当前功能') {
        if (!this.hasMerchantRecord) {
          this.setRecentActionSummary('高权限入口校验失败', '当前账号尚未完成商家入驻');
          uni.showModal({
            title: '提示',
        ...
        if (!this.isMerchantSuper) {
          this.setRecentActionSummary('高权限入口校验失败', `${featureName}仅商家超管可用`);
          uni.showToast({
            icon: 'none',
        ...
        return true;
			...
			gridTap(item) {
        this.setRecentActionSummary('打开工具入口', item.displayName || item.name || item.code || '工具');
        if (this.reviewMode) {
      ...
      //页面跳转
      jump(url) {
        this.setRecentActionSummary('跳转页面', url);
        uni.navigateTo({
          url: url
        });
      },
	...
	.zaiui-my-box {
      ...
      .zaiui-head-glow {
        ...
        &--one {
          ...
          right: -70rpx;
          top: 64rpx;
          background: radial-gradient(circle, rgba(255,255,255,0.88) 0%, rgba(255,255,255,0.08) 70%, rgba(255,255,255,0) 100%);
        }
        &--two {
          width: 220rpx;
          height: 220rpx;
        ...
        .recent-release-panel {
          ...
          padding: 24rpx 26rpx;
          border-radius: 24rpx;
          background: linear-gradient(180deg, rgba(255, 250, 242, 0.99) 0%, rgba(255, 244, 232, 0.98) 100%);
        ...
        .workbench-entry-panel {
          ...
          padding: 24rpx 26rpx;
          border-radius: 24rpx;
          background: linear-gradient(180deg, rgba(244, 248, 255, 0.99) 0%, rgba(233, 241, 252, 0.98) 100%);
          ...
          .order-grid-card {
            ...
            border-radius: 24rpx;
            min-height: 140rpx;
            background: linear-gradient(180deg, rgba(255, 253, 248, 0.99) 0%, rgba(252, 248, 240, 0.98) 100%);
            ...
            &.is-all .order-grid-icon {
              background: linear-gradient(135deg, rgba(255, 166, 177, 0.18) 0%, rgba(255, 222, 195, 0.22) 100%);
              .text-xxl {
                color: #e96d7f !important;
              }
            }
            &.is-pay .order-grid-icon {
              background: linear-gradient(135deg, rgba(252, 213, 137, 0.24) 0%, rgba(255, 239, 208, 0.22) 100%);
              .text-xxl {
                color: #d88c26 !important;
              }
            }
            &.is-receipt .order-grid-icon {
              background: linear-gradient(135deg, rgba(255, 220, 167, 0.2) 0%, rgba(255, 242, 214, 0.22) 100%);
              .text-xxl {
                color: #dc9142 !important;
              }
            }
            &.is-evaluate .order-grid-icon {
              background: linear-gradient(135deg, rgba(226, 204, 255, 0.18) 0%, rgba(255, 238, 214, 0.2) 100%);
              .text-xxl {
          ...
          .order-grid-hint {
            ...
            color: #a1876f;
				...
				.merchant-service-card {
					...
					padding: 29.09rpx;
					border-radius: 30rpx;
					background: linear-gradient(135deg, rgba(32, 79, 118, 0.96) 0%, rgba(236, 138, 87, 0.9) 100%);
					color: #ffffff;
          ...
          border-radius: 30rpx;
          background: linear-gradient(135deg, rgba(21, 75, 114, 0.96) 0%, rgba(236, 138, 87, 0.94) 100%);
          color: #ffffff;
          box-shadow: 0 18rpx 42rpx rgba(21, 33, 52, 0.08);
          .pending-badge {
            display: inline-flex;
            ...
            text-align: center;
              ...
              justify-content: center;
				...
				.merchant-super-tool-card {
					...
					align-items: flex-start;
					min-height: 220rpx;
					animation: superToolEnter 0.55s ease-out both, superToolFloat 4.6s ease-in-out infinite;
					&:nth-child(1) {
					...
					&.is-primary {
						...
						animation: superToolEnter 0.55s ease-out both, superToolFloatStrong 3.8s ease-in-out infinite;
					}
					&.is-secondary {
