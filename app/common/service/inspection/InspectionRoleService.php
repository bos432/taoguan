...
class InspectionRoleService
{
    ...
    {
        $info = InspectionRoleCache::get($id);
        if (empty($info)) {
            $model = new InspectionRoleModel();
            ...
            InspectionRoleCache::set($id, $info);
        }
    ...
    public static function edit($ids, $param = [])
    {
        ...
        try {
            ...
            // 修改
            $model->where('ins_id',$ins_id)->where($pk, 'in', $ids)->update($param);
    ...
    public static function dele($ids, $real = false)
    {
        ...
        try {
            ...
            if ($real) {
                ...
                $model->where('is_admin','<>',1)->where('ins_id',$ins_id)->where($pk, 'in', $ids)->delete();
            } else {
                $update = delete_update();
                $model->where('is_admin','<>',1)->where('ins_id',$ins_id)->where($pk, 'in', $ids)->update($update);
            }
            // 提交事务
            $model->commit();
        } catch (\Exception $e) {
    ...
    {
        return InspectionUserService::list($where, $page, $limit, $order, $field);
    }
    ...
    public static function menu_ids($role_id, $where = [])
    {
        ...
        $RoleModel = new InspectionRoleModel();
