 ...
 * 文件标签验证器
 */
class InspectionTagValidate extends Validate
{
    // 验证规则
    protected $rule = [
        'ids'      => ['require', 'array'],
    ...
    protected function checkExisted($value, $rule, $data = [])
    {
        ...
        if ($tag_unique) {
            ...
            $where = [[$pk, '<>', $id],['ins_id', '=', ins_id()], ['tag_unique', '=', $tag_unique], where_delete()];
        ...
        $where = [[$pk, '<>', $id],['ins_id', '=', ins_id()], ['tag_name', '=', $data['tag_name']], where_delete()];
        $info = $model->field($pk)->where($where)->find();
        if ($info) {
    ...
    {
        // $info = InspectionTagsModel::where('tag_id', 'in', $data['ids'])->find();
        // if ($info) {
