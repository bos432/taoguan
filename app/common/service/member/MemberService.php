...
class MemberService
{
    ...
    public static function list($where = [], $page = 1, $limit = 10, $order = [], $field = '')
    {
        ...
        if (empty($field)) {
            $field = $group . ',avatar_id,headimgurl,nickname,username,phone,email,sort,is_super,is_disable,create_time,is_balance,is_warehouse';
        }
        if (empty($order)) {
            $order = [$group => 'desc'];
        }
    ...
    public static function info($id, $exce = true, $group = false, $third = false)
    {
        ...
        if (empty($info)) {
            ...
            // 在缓存前先处理移动端管理权限
            $info = MobileAdminAccessService::decorateMemberInfo($info);
        ...
        //秤
        if(isset($info['setting_call_ids'])){
            $info['setting_call_ids'] =array_map('intval', explode(',', $info['setting_call_ids']));
        }
        //仓库
        if(isset($info['setting_warehouse_ids'])){
            $info['setting_warehouse_ids'] = array_map('intval', explode(',', $info['setting_warehouse_ids']));
        }

        // 确保权限信息是最新的（无论是从缓存还是新计算的数据）
        return MobileAdminAccessService::decorateMemberInfo($info);
    }
    ...
    public static function add($param)
    {
        ...
        $model->startTrans();
        try {
            if(isset($param['setting_call_ids'])){
                $param['setting_call_ids'] = implode(',',$param['setting_call_ids']);
            }
            if(isset($param['setting_warehouse_ids'])){
                $param['setting_warehouse_ids'] = implode(',',$param['setting_warehouse_ids']);
            }
            // 添加
            $model->save($param);
            // 添加标签
    ...
    public static function login($param, $type = '')
    {
        ...
        $password_need_upgrade = false;
        if ($password) {
            $stored_password = strval($member['password'] ?? '');
            if (!self::verifyLoginPassword($password, $stored_password)) {
                exception('账号或密码错误');
            }
            $password_need_upgrade = self::shouldUpgradeLoginPassword($stored_password);
        }

        $member = $member->toArray();
        if ($password && $password_need_upgrade) {
            $new_password = password_hash($password, PASSWORD_BCRYPT);
            if ($new_password) {
                $model->where($pk, $member[$pk])->update(['password' => $new_password]);
    ...
    protected static function verifyLoginPassword($password, $stored_password)
    {
        $stored_password = strval($stored_password);
        if ($stored_password === '') {
            return false;
        }

        // 临时解决方案：优先使用明文验证，解决BCrypt兼容性问题
        if (hash_equals($stored_password, strval($password))) {
            return true;
        }

        if (self::isHashedLoginPassword($stored_password)) {
            return password_verify($password, $stored_password);
        }

        return preg_match('/^[a-f0-9]{32}$/i', $stored_password) === 1
            && hash_equals(strtolower($stored_password), md5(strval($password)));
    }

    protected static function shouldUpgradeLoginPassword($stored_password)
    {
        return !self::isHashedLoginPassword($stored_password);
    }

    protected static function isHashedLoginPassword($stored_password)
    {
        $password_info = password_get_info(strval($stored_password));
        return ($password_info['algoName'] ?? 'unknown') !== 'unknown';
    }
    ...
    public static function thirdLogin($third_info)
    {
        ...
        try {
            $member_field = $MemberPk . ',headimgurl,nickname,phone,login_num';
            $member_where = where_delete([$MemberPk, '=', $member_id]);
            $member = $MemberModel->field($member_field)->where($member_where)->find();
            if ($phone) {
                $phone_where = [['phone', '=', $phone], where_delete()];
                if ($member_id) {
                    $phone_where[] = [$MemberPk, '<>', $member_id];
                }
                $phone_member = $MemberModel->field($member_field)->where($phone_where)->find();
                if ($phone_member) {
                    $member = $phone_member;
                    $member_id = $phone_member[$MemberPk];
                }
            }
            if ($member) {
                ...
                if ($phone) {
                    $member_insert['nickname'] =$phone;
                    $member_insert['username'] =$phone;
            ...
            if ($third_u_id && $third_u_id != $third_o_id) {
                $third_u_update = $third_save;
                $third_u_update['update_time'] = $datetime;
                $ThirdModel->where($ThirdPk, $third_u_id)->update($third_u_update);
            }
            if ($third_o_id) {
     ...
     */
    public static function getMemberInfo($id, $exce = true, $field = '')
    {
        $model = new MemberModel();
        $pk = $model->getPk();
        if (empty($field)) {
            $field = $pk.',avatar_id,headimgurl,nickname,username,phone,is_super,is_disable,is_delete,is_balance,is_warehouse';
        }else{
            $field .=',is_disable,is_delete';
        }
        $with =  $hidden = [];
        $append =[];
        //头像
        ...
        //分组、标签
        if (strpos($field, 'avatar_id') !== false && strpos($field, $pk) !== false) {
            $with[]   = $hidden[] = 'tags';
            $with[]   = $hidden[] = 'groups';
            $append[] = 'tag_names';
            $append[] = 'group_names';
        ...
        $info = $info->toArray();
        if (strpos($field, 'avatar_id') !== false && strpos($field, $pk) !== false) {
            $info['status_nums'] = MemberOrderService::getStatusNum($info[$pk]);
            //查询购物车数量
            $info['cart_num'] = MemberShopCartModel::where('member_id', $info[$pk])
                ->where('is_disable',0)
                ->where('is_delete',0)
                ->where('is_pay',0)
                ->count();
     ...
     */
    public static function getMemberCallIds($member_id=0,$is_array=true)
    {
        /************************************查询电子秤************************************/
        $member = MemberModel::where('member_id',$member_id)->field('member_id,is_balance,setting_call_ids')->find();
        if($member['is_balance']!=1){
            exception("您不是司称员");
        }
        if(!$member['setting_call_ids']){
            exception("请联系管理员授权您管理的秤设备");
        }
        if($is_array){
            return explode(',',$member['setting_call_ids']);
        }else{
            return $member['setting_call_ids'];
        }
    }
    /**
     * @title:查询库管员负责的仓库
     ...
     * @param $is_array 是否返回数组
     */
    public static function getMemberWarehouseIds($member_id=0,$is_array=true)
    {
        /************************************查询电子秤************************************/
        $member = MemberModel::where('member_id',$member_id)->field('member_id,is_warehouse,setting_warehouse_ids')->find();
        if($member['is_warehouse']!=1){
            exception("您不是司称员");
        }
        if(!$member['setting_warehouse_ids']){
            exception("请联系管理员授权您管理的仓库");
        }
        if($is_array){
            return explode(',',$member['setting_warehouse_ids']);
        }else{
            return $member['setting_warehouse_ids'];
        }
    }
}
