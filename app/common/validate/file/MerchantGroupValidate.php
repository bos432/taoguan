 ...
 * 文件分组验证器
 */
class MerchantGroupValidate extends Validate
{
    // 验证规则
    protected $rule = [
        'ids'        => ['require', 'array'],
    ...
    protected function checkExisted($value, $rule, $data = [])
    {
        ...
        if ($group_unique) {
            ...
            $where = [[$pk, '<>', $id],['mer_id', '=', mer_id()], ['group_unique', '=', $group_unique], where_delete()];
        ...
        $where = [[$pk, '<>', $id],['mer_id', '=', mer_id()], ['group_name', '=', $data['group_name']], where_delete()];
        $info = $model->field($pk)->where($where)->find();
        if ($info) {
    ...
    protected function checkFile($value, $rule, $data = [])
    {
        ...
        // $info = MerchantFileModel::field('group_id')->where($where)->find();
        // if ($info) {
