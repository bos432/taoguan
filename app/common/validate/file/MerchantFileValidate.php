 ...
 * 文件管理验证器
 */
class MerchantFileValidate extends Validate
{
    // 验证规则
    protected $rule = [
        'ids'       => ['require', 'array'],
    ...
    protected function checkTagGroup($value, $rule, $data = [])
    {
        ...
        // $info = MerchantFileModel::field('file_id')->where($where)->find();
        // if ($info) {
        //     return '文件存在分组，请解除后再删除：' . $info['file_id'];
        // }

        // $info = MerchantTagsModel::field('file_id')->where('file_id', 'in', $data['ids'])->find();
        // if ($info) {
