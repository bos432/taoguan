...
use app\common\cache\merchant\MerchantMenuCache;
...
class MerchantMenuService
{
    ...
    public static function list($type = 'tree', $where = [], $order = [], $field = '')
    {
        ...
        $data = MerchantMenuCache::get($key);
        if (empty($data)) {
            $append = [];
            ...
            MerchantMenuCache::set($key, $data);
        }
    ...
    public static function add($param)
    {
        ...
        $model->startTrans();
        try {
            $menu = MerchantMenuModel::create($param);
            $id = $menu->$pk;
    ...
    {
        return MerchantRoleService::list($where, $page, $limit, $order, $field);
    }
    ...
    public static function roleRemove($menu_id, $role_ids = [])
    {
        ...
        if (empty($role_ids)) {
            $role_ids = MerchantRoleMenusModel::where($where)->column('role_id');
        }
        $where[] = ['role_id', 'in', $role_ids];

        $res = MerchantRoleMenusModel::where($where)->delete();
        $user_ids = MerchantUserModel::select()->column('mer_user_id');

        MerchantRoleCache::del($role_ids);
    ...
    {
        $key = $type;
        $list = MerchantMenuCache::get($key);
        if (empty($list)) {
            $model = new MerchantMenuModel();
            ...
            MerchantMenuCache::set($key, $list);
        }
    ...
    public static function unrateList($type = 'url')
    {
        ...
        if (empty($list)) {
            ...
            $column = 'menu_url';
            $menu_is_unrate = Config::get('merchant.menu_is_unrate', []);
            if ($type == 'id') {
                $column = $model->getPk();
                if ($menu_is_unrate) {
