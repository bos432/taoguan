...
function server_url()
{
    ...
    if (stripos($domain, 'http://') === 0) {
        ...
        $httpsFlag = Request::server('HTTPS');
        if (is_string($forwardedProto) && strtolower($forwardedProto) === 'https') {
            $domain = preg_replace('/^http:\/\//i', 'https://', $domain);
        } elseif (is_string($httpsFlag) && strtolower($httpsFlag) === 'on') {
            $domain = preg_replace('/^http:\/\//i', 'https://', $domain);
        } elseif (Request::server('SERVER_PORT') == 443) {
            $domain = preg_replace('/^http:\/\//i', 'https://', $domain);
        }
    }
    return $domain;
...
{
    $data['is_delete']   = 1;
    $data['delete_uid']  = operate_user_id();
 ...
 * @return Exception
 */
function ins_user_token_verify($user_token = '')
{
    if (empty($user_token)) {
        $user_token = ins_user_token();
    }

    InspectionUserTokenService::verify($user_token);
}
/**
 * 检测机构ID
 *
 ...
 * @return int
 */
function ins_id($exce = false)
{
    $user_token = ins_user_token();

    return InspectionUserTokenService::insId($user_token, $exce);
}
 ...
 * @return int
 */
function ins_user_id($exce = false)
{
    $user_token = ins_user_token();

    return InspectionUserTokenService::insUserId($user_token, $exce);
}
/**
 * 检测机构token
 *
 * @return string
 */
function ins_user_token()
{
    $system = Config::get('Inspection');
    if ($system['token_type'] == 'header') {
        $user_token = Request::header($system['token_name'], '');
    } else {
        $user_token = Request::param($system['token_name'], '');
 ...
 * @return array
 */
function ins_user_super_ids()
{
    return Config::get('Inspection.super_ids', []);
}
/**
 * 检测机构用户是否系统超管
 *
 ...
 * @return bool
 */
function ins_user_is_super($user_id = 0)
{
    if (empty($user_id)) {
        return false;
    }

    $user_super_ids = ins_user_super_ids();
    if (empty($user_super_ids)) {
        return false;
    }
    if (in_array($user_id, $user_super_ids)) {
 ...
 * @return array
 */
function ins_user_hide_where($user_id_field = 'ins_user_id')
{
    $super_hide = Config::get('Inspection.super_hide', false);
    if ($super_hide) {
        $user_id = ins_user_id();
 ...
 * @return array
 */
function ins_delete_update($data = [], $field = '')
{
    $data['is_delete']   = 1;
    $data['delete_uid']  = ins_id();
    $data['delete_time'] = datetime();
