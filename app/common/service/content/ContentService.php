...
class ContentService
{
    ...
    public static function list($where = [], $page = 1, $limit = 10,  $order = [], $field = '')
    {
        ...
        if (empty($field)) {
            $field = $group . ',unique,image_id,title,release_time,hits,is_top,is_hot,is_rec,is_disable,create_time,update_time';
        }
        if (empty($order)) {
     ...
     * @date：2024/12/9
     */
    public static function getListByHome()
    {
        $categoryModel = new CategoryModel();
        $model = new ContentModel();
        $type_list = $categoryModel
            ->where('is_disable',0)
            ->where('is_delete',0)
            ->field('category_id as value,category_name as label')
        ...
        foreach ($type_list as $k => $v) {
            $type_list[$k]['label'] = CategoryService::normalizeCategoryName(
                $v['label'] ?? '',
                intval($v['value'] ?? 0)
            );
            $type_list[$k]['list'] =$model
                ->alias('m')
                ->join('content_attributes c', 'm.content_id=c.content_id')
                ->with($with)->append($append)->hidden($hidden)
                ->where('is_disable',0)
                ->where('is_delete',0)
                ->where('c.category_id',$v['value'])
                ->page(1)
                ->limit(10)
                ->field('m.content_id,image_id,title,create_time,source,hits_initial')
                ->select()->toArray();
            foreach ($type_list[$k]['list'] as $k1 => $v1) {
                $type_list[$k]['list'][$k1]['text'] =$v1['hits_initial']."人阅读";
                if(isset($v1['source']) && $v1['source']){
                    $type_list[$k]['list'][$k1]['text'] =$v1['source'];
                }
            }
        }
        return $type_list;
    ...
    public static function info($id, $exce = true)
    {
        ...
        $model->where($pk, $id)->inc('hits_initial', 1)->update();
        ContentCache::del($id);
        return $info;
    }
    ...
    public static function statistic()
    {
        ...
        if (empty($data)) {
            ...
            foreach ($categorys as $v) {
                $x_data[] = CategoryService::normalizeCategoryName(
                    $v['category_name'] ?? '',
                    intval($v['category_id'] ?? 0)
                );
        ...
        if (!empty($data['x_data']) && is_array($data['x_data'])) {
            foreach ($data['x_data'] as $index => $label) {
                $categoryId = 0;
                if (!empty($data['category_ids'][$index] ?? null)) {
                    $categoryId = intval($data['category_ids'][$index]);
                }
                $data['x_data'][$index] = CategoryService::normalizeCategoryName($label, $categoryId);
            }
        }
