 ...
 * 菜单管理验证器
 */
class InspectionMenuValidate extends Validate
{
    // 验证规则
    protected $rule = [
        'ids'       => ['require', 'array'],
    ...
    protected function checkChild($value, $rule, $data = [])
    {
        ...
        $info = InspectionMenuModel::field('menu_pid')->where($where)->find();
        if ($info) {
    ...
    {
        // $info = InspectionRoleMenusModel::where('menu_id', 'in', $data['ids'])->find();
        // if ($info) {
