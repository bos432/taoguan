...
use app\common\cache\file\MerchantFileCache;
...
class MerchantFileService
{
    ...
    public static function add($param)
    {
        ...
        if ($type == 'url') {
            ...
            $file_exist = $model->field($pk)->where('mer_id',$param['mer_id'])->where('domain', $param['domain'])->where('file_path', $param['file_path'])->find();
        } else {
            $file = $param['file'];
            unset($param['file']);
            ...
            $storage_subdir = 'merchant' . $param['mer_id'] . '/file/' . date('Ymd');
            ...
            $file_exist = $model->field($pk)->where('mer_id',$param['mer_id'])->where('file_hash', $file_hash)->find();
            if ($file_exist) {
    ...
    public static function edit($ids, $param = [])
    {
        ...
        try {
            ...
            // 修改
            $model->where($pk, 'in', $ids)->where('mer_id',$mer_id)->update($param);
        ...
        MerchantFileCache::del($ids);
        MerchantFileCache::del($unique);
    ...
    public static function dele($ids, $real = false)
    {
        ...
        try {
            ...
            $mer_id = mer_id();
            if ($real) {
                $file = $model->field($pk . ',file_path')->where('mer_id',$mer_id)->where($pk, 'in', $ids)->select();
                foreach ($file as $v) {
    ...
    public static function statistic()
    {
        ...
        $data = MerchantFileCache::get($key);
        if (empty($data)) {
            $model = new MerchantFileModel();
            $mer_id = mer_id();
            $file_types = SettingService::fileTypes();
            $file_field = 'file_type,count(file_type) as count';
            $file_count = $model->where('mer_id',$mer_id)->field($file_field)->where([where_delete()])->group('file_type')->select()->toArray();
            ...
            $data['count'] = $model->where('mer_id',$mer_id)->where([where_delete()])->count();

            MerchantFileCache::set($key, $data);
        }
