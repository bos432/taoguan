...
{
    private function handleMiniappPhoneCodeFailure(\Throwable $e)
    {
        $message = strtolower(trim((string) $e->getMessage()));
        $keywords = ['login_code_require', 'invalid code', 'code been used'];
        foreach ($keywords as $keyword) {
            if ($keyword !== '' && strpos($message, $keyword) !== false) {
                return error('微信手机号授权已失效，请重新点击授权登录');
            }
        }
    ...
    public function miniapp()
    {
        ...
        if ($param['phone_code'] ?? '') {
            try {
                ...
                return $this->handleMiniappPhoneCodeFailure($e);
            }
        }
     ...
     * @Apidoc\NotParams()
     */
    public function miniappByPhone()
    {
        $param    = $this->params([
            'app/s' => 'wx',
            'login_code/s' => '',
        ...
        $validate = Validate::rule(['app' => 'require', 'login_code' => 'require','phone_code' => 'require', 'userinfo' => 'array']);
        if (!$validate->check($param)) {
            return error($validate->getError());
        }
        ...
        $user_info                = $miniapp->login($param['login_code']);
        try {
            $phone = $miniapp->getPhoneNumber($param['phone_code']);
        } catch (\Throwable $e) {
            return $this->handleMiniappPhoneCodeFailure($e);
