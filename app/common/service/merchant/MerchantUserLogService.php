...
use app\common\cache\merchant\MerchantUserLogCache;
use app\common\model\merchant\MerchantUserLogModel;
...
class MerchantUserLogService
{
    ...
    {
        $model = new MerchantUserLogModel();
        $pk = $model->getPk();

        if (empty($field)) {
            $field = $pk . ',mer_id,mer_user_id,menu_id,request_url,request_method,request_ip,request_region,request_isp,response_code,response_msg,create_time';
        }
        if (empty($order)) {
            $order = [$pk => 'desc'];
        }
        ...
        if (mer_user_hide_where()) {
            $where[] = mer_user_hide_where();
        }
        $where[] =['mer_id','=',mer_id()];
        $with = $append = $hidden = $field_no = [];
        if (strpos($field, 'mer_user_id') !== false) {
            $with[]   = $hidden[] = 'user';
            $append[] = 'nickname';
            $append[] = 'username';
        }
    ...
    {
        $info = MerchantUserLogCache::get($id);
        if (empty($info)) {
            $model = new MerchantUserLogModel();
            ...
            MerchantUserLogCache::set($id, $info);
        }
    ...
    public static function add($param = [], $log_type = SettingService::LOG_TYPE_OPERATION)
    {
        ...
        if (user_log_switch()) {
            ...
            $param_without = Config::get('merchant.log_param_without', []);
            array_push($param_without, Config::get('merchant.token_name'));
            foreach ($param_without as $v) {
                unset($request_param[$v]);
            }

            $menu       = MerchantMenuService::info('', false);
            $ip_info    = Utils::ipInfo();
            $user_agent = $_SERVER['HTTP_USER_AGENT'] ?? Request::header('user-agent') ?? '';

            $param['mer_id']         = mer_id();
            $param['mer_user_id']         = mer_user_id();
    ...
    public static function edit($ids, $param = [])
    {
        ...
        $res = $model->where('mer_id',mer_id())->where($pk, 'in', $ids)->update($param);
        if (empty($res)) {
            exception();
        }
    ...
    public static function dele($ids, $real = false, $user_id = '')
    {
        ...
        $where = [[$pk, 'in', $ids]];
        if ($user_id !== '') {
            $where[] = ['mer_user_id', 'in', $user_id];
        }
        if ($real) {
            $res = $model->where('mer_id',$mer_id)->where($where)->delete();
        } else {
            $update = delete_update();
            $res = $model->where('mer_id',$mer_id)->where($where)->update($update);
        }
    ...
    public static function clear($where = [])
    {
        ...
        $count = $model->where('mer_id',mer_id())->where($where)->delete(true);
    ...
    public static function clearLog()
    {
        ...
        if ($setting['log_save_time']) {
            ...
            if (0 <= $time && $time <= 8) {
                $key = 'clear';
                $val = MerchantUserLogCache::get($key);
                if (empty($val)) {
                    $days = $setting['log_save_time'];
                    $date = date('Y-m-d H:i:s', strtotime("-{$days} day"));
                    $where = [['create_time', '<', $date]];
                    $res = MerchantUserLogService::clear($where);
                    $res['where'] = $where;
                    ...
                    MerchantUserLogCache::set($key, $days, 1800);
                }
            }
        }
    }
