...
class Member extends BaseController
{
    ...
    public function edit()
    {
        ...
        switch ($param['type']){
            case 1:
                //修改姓名
                validate(MemberValidate::class)->scene('editName')->check($param);
                $edit_data['name'] =$param['name'];
                break;
            case 2:
                //修改性别
                validate(MemberValidate::class)->scene('editGender')->check($param);
                $edit_data['gender'] =$param['gender'];
                break;
        }
        unset($param['type']);
        ...
        $data = MemberService::edit($param['member_id'], $edit_data);
    ...
    public function memberInfo()
    {
        ...
        $param['member_id'] = member_id(true);
        validate(MemberValidate::class)->scene('info')->check($param);
        $data = MemberService::getMemberInfo($param['member_id'], true, $param['field']);
        if ($data['is_disable'] == 1) {
            return error('您已被禁用');
        } else if ($data['is_delete'] == 1) {
            return error('您已被注销');
        ...
        $data['wx_approved'] = isset($settingInfo['wx_approved']) ? $settingInfo['wx_approved'] : 0;
        $data['admin_next_host'] = rtrim(server_url(), '/') . '/admin-next/';
        //判断是否是商家
        $mer_count = MerchantModel::where('member_id', $param['member_id'])
            ->where('is_disable',0)
            ->where('is_delete',0)
            ->where('auth_state',1)
            ->count();
        $data['is_merchant'] = $mer_count>0?1:0;
        return success($data);
    }

    /**
     * @Apidoc\Title("会员设置启动数据")
     */
    ...
    private function runtimeData(): array
    {
        ...
        return MemberBootstrapService::bootstrap($param['member_id'], $param['preference_group'], $param['preference_keys']);
    }
}
