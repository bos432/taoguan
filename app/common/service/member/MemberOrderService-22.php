...
//        var_dump($model->with($with)->append($append)->hidden($hidden)->field($field)->where($where)->page($page)->limit($limit)->order($order)->buildSql());exit();
    ...
    public static function getStatusNum($member_id=0){
        ...
        $status_nums = array();
        $status_nums['all'] = (clone $query)->count();
    ...
    public static function submitService($ids,$param = [])
    {
        ...
        try {
            ...
            foreach ($list as $key=>$val) {
                $val->refund_status = 1;
                $val->refund_type = isset($param['refund_type'])?$param['refund_type']:2;
                $val->refund_price = isset($param['refund_price'])?$param['refund_price']:$val['total_price'];
                $val->refund_reason_wap_explain = isset($param['refund_reason_wap_explain'])?$param['refund_reason_wap_explain']:null;
                $val->refund_reason_wap_img_ids =isset($param['refund_reason_wap_imgs'])?implode(",",array_column($param['refund_reason_wap_imgs'],'file_id')):null;
                $val->status = MemberOrderModel::getStatus('service',1);
    ...
    public static function serviceOrder($id,$param = [],$source=1)
    {
        ...
        try {
            ...
            if($is_refund){
                ...
                $response = $app->refund->byOutTradeNumber($info['pay_common_on'], $out_refund_no, ($info['total_price']*100), ($save_data['refund_price']*100), [
    ...
    public static function returnGoods($ids,$param = [])
    {
        ...
        try {
            ...
            foreach ($list as $key=>$val) {
                $val->refund_delivery_id=$param['refund_delivery_id'];
                $val->refund_express=$param['refund_express'];
                $val->refund_express_name=$delivery_obj['title'];
                $val->refund_express_code=$delivery_obj['code'];
                $val->update_time = datetime();
                //订单日志
                $log = MemberOrderLogService::add([
                    'title'=>'买家已发货',
