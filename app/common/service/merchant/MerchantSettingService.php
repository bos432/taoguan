...
use app\common\cache\merchant\MerchantSettingCache;
use app\common\model\merchant\MerchantSettingModel;
...
class MerchantSettingService
{
    ...
    public static function info($fields = '')
    {
        ...
        $info = MerchantSettingCache::get($key);
        if (empty($info)) {
            $model = new MerchantSettingModel();
            $pk = $model->getPk();
            ...
            $info['token_type'] = Config::get('merchant.token_type');
            $info['token_name'] = Config::get('merchant.token_name');
            $info['token_exps'] = $info['token_exp'] * 3600;

            MerchantSettingCache::set($key, $info);
        }
