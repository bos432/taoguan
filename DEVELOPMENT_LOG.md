...
## 2026-08-25 渐进式重构阶段三：平台商家高风险菜单权限

- 阶段名称：渐进式重构阶段三：平台商家高风险菜单权限
- 本阶段完成内容：新增版本化平台菜单迁移，在现有商家管理父菜单下增加“绑定/解绑商家会员”“授予/取消商家超管”“查看商家授权记录”三个隐藏按钮权限。迁移使用 `NOT EXISTS` 可重复执行，全部保持登录必需且不写 `system_role_menus`，不会自动扩权任何普通角色；管理员可在现有角色菜单中显式分配。稳定权限上下文继续将三个 URL 映射为 `platform.merchant.bind`、`platform.merchant.super_authorize` 和 `platform.merchant.view`，控制器仍执行后端断言。
- 修改/新增的主要文件：`private/migrations/20260825_add_merchant_authorization_menus.sql`、`tests/refactor/merchant-authorization-menu-database-integration.php`、`DEVELOPMENT_LOG.md`
- 运行或测试结果：菜单权限数据库集成测试通过 13 项断言，确认三个 URL 唯一存在、父级为商家管理、隐藏且不免登录、角色关联为 0；事务内临时普通平台用户无绑定/超管权限并返回统一 HTTP 403，平台超管具备两项权限，临时用户完整回滚。完整 `tests/refactor` 32 个脚本共 441 项断言通过；本地迁移后新增菜单 3 条、角色关联 0 条，核心业务数量保持不变，操作请求、订单事件、网关尝试和商家授权日志均为 0；`git diff --check` 通过。
- 遗留问题：灰度及正式环境需执行本菜单迁移并清理 `system_menu`、`system_user` 缓存；迁移故意不替普通角色授权，运营负责人需要按权限矩阵决定哪些审核/财务角色可查看记录、绑定会员或授予超管。尚未自动验证普通平台角色显式分配单项菜单后的最小权限和 `permission_version` 刷新。
- 下一阶段应继续处理的事项：重新读取计划和日志后，新增平台普通角色最小权限与刷新测试；事务内创建角色/用户并逐项分配商家查看、绑定和超管菜单，验证未分配权限 403、分配后仅获得对应权限、撤销后权限版本变化且旧缓存不会继续生效。

## 2026-08-25 渐进式重构阶段三：平台角色权限即时刷新

- 阶段名称：渐进式重构阶段三：平台角色权限即时刷新
- 本阶段完成内容：修复平台 `RoleService` 修改或删除角色后只清角色缓存、未清关联用户缓存的问题；现在按 `system_user_attributes.role_id` 精确找到受影响用户并清除 `system_user` 缓存，避免角色撤权后旧菜单 URL 和统一权限继续生效最长 12 小时。未采用全量缓存清理。新增事务内最小角色测试，验证普通角色只获得显式分配的稳定权限码，权限增减会立即重建上下文并更新 `permission_version`。
- 修改/新增的主要文件：`app/common/service/system/RoleService.php`、`tests/refactor/platform-role-permission-refresh-database-integration.php`、`DEVELOPMENT_LOG.md`
- 运行或测试结果：平台角色权限刷新数据库集成测试通过 9 项断言，覆盖初始仅查看、显式增加绑定、未旁带超管授权、撤销绑定、权限版本变化及撤权后统一 HTTP 403；事务内角色和用户完整回滚。完整 `tests/refactor` 33 个脚本共 450 项断言通过；测试结束后平台正式用户仍为 1、临时用户为 0，核心业务数量保持不变，操作请求、订单事件、网关尝试和商家授权日志均为 0；PHP 语法和 `git diff --check` 通过。
- 遗留问题：本阶段只修复平台角色缓存失效；商家桌面端 `MerchantRoleService` 和巡检端 `InspectionRoleService` 仍需检查相同问题。平台前端已能在重新请求权限上下文后获得新版本，但浏览器端是否在角色保存后主动刷新当前登录用户上下文还需前端迁移阶段验证。
- 下一阶段应继续处理的事项：重新读取计划和日志后，统一修复商家角色与巡检角色菜单变更后的关联用户缓存失效；分别用事务内最小角色验证只获得本机构/本商家的权限、撤权后 `permission_version` 变化且后端立即 403。

## 2026-08-25 渐进式重构阶段三：商家与巡检角色权限即时刷新

- 阶段名称：渐进式重构阶段三：商家与巡检角色权限即时刷新
- 本阶段完成内容：修复商家端与巡检端角色编辑、删除后只清角色缓存而未清关联用户缓存的问题；现在按各自角色关系表精确清除受影响用户缓存，撤权后不会继续使用旧权限。同步修复两个 `userRemove()` 错查不存在的 `user_id` 字段，分别改用 `mer_user_id` 和 `ins_user_id`。缓存失效入口保持独立可验证，正式编辑、删除路径均调用该入口，租户范围校验未放宽。
- 修改/新增的主要文件：`app/common/service/merchant/MerchantRoleService.php`、`app/common/service/inspection/InspectionRoleService.php`、`tests/refactor/merchant-inspection-role-permission-refresh-database-integration.php`、`DEVELOPMENT_LOG.md`
...
## 2026-08-25 渐进式重构阶段三：巡检前端源码基线核查

- 阶段名称：渐进式重构阶段三：巡检前端源码基线核查
- 本阶段完成内容：核查当前工作区、全部 Git 历史分支、前端构建目录及发布配置，确认仓库仅包含 `app/inspection` 巡检后端、admin-next 巡检管理页面和商家端巡检 API，没有独立巡检前端源码目录、前端 `package.json` 或 `public/inspection` 入口。未把商家端复制为巡检端，避免在缺少原始入口、页面范围和部署约定时擅自新建整套前端。
- 修改/新增的主要文件：`DEVELOPMENT_LOG.md`
