<template>
  <el-row style="width: 100%">
    <el-col :span="24">
      <el-col :span="12" v-if="fileTip">
        <el-text size="default" truncated :title="fileTip">{{ fileTip }}</el-text>
      </el-col>
      <el-col :span="12">
        <el-button @click="fileUpload()" v-if="fileSource!='list'">{{ uploadBtn }}</el-button>
        <el-button @click="fileDelete('all')" v-if="fileList.length>0 && fileSource!='list'">{{ deleteBtn }}</el-button>
      </el-col>
    </el-col>
    <el-col v-for="(item, index) in fileList" :key="index" :span="6" style="margin-top: 10px;">
      <el-row class="file-row">
        ...
        <el-col style="max-height: 40px">
          ...
          <el-link class="file-link" type="primary" :underline="false" title="删除" @click="fileDelete(index)" v-if="fileSource!='list'">
