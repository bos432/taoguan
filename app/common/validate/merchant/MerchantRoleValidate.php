 ...
 * 角色管理验证器
 */
class MerchantRoleValidate extends Validate
{
    // 验证规则
    protected $rule = [
        'ids'       => ['require', 'array'],
    ...
    {
        // $menu = MerchantRoleMenusModel::where('role_id', 'in', $data['ids'])->find();
        // if ($menu) {
        //     return '角色下存在菜单，请在[菜单]或[修改]中解除后再删除：' . $menu['role_id'];
        // }

        // $user = MerchantUserAttributesModel::where('role_id', 'in', $data['ids'])->find();
        // if ($user) {
