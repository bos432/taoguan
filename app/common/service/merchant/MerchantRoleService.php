...
use app\common\model\merchant\MerchantRoleModel;
use app\common\model\merchant\MerchantUserAttributesModel;
...
class MerchantRoleService
{
    ...
    {
        $model = new MerchantRoleModel();
        $pk = $model->getPk();

        if (empty($field)) {
            $field = 'm.' . $pk . ',is_admin,role_name,role_desc,remark,sort,is_disable,create_time,update_time';
        }
        if (empty($order)) {
    ...
    public static function edit($ids, $param = [])
    {
        ...
        try {
            ...
            // 修改
            $model->where('mer_id',$mer_id)->where($pk, 'in', $ids)->update($param);
            if (var_isset($param, ['menu_ids'])) {
                foreach ($ids as $id) {
                    $info = $model->where('mer_id',$mer_id)->with(['menus'])->find($id);
                    // 修改菜单
                    if (isset($param['menu_ids'])) {
    ...
    public static function dele($ids, $real = false)
    {
        ...
        try {
            ...
            if ($real) {
                ...
                $model->where('is_admin','<>',1)->where('mer_id',$mer_id)->where($pk, 'in', $ids)->delete();
            } else {
                $update = delete_update();
                $model->where('is_admin','<>',1)->where('mer_id',$mer_id)->where($pk, 'in', $ids)->update($update);
            }
            // 提交事务
            $model->commit();
        } catch (\Exception $e) {
    ...
    {
        return MerchantUserService::list($where, $page, $limit, $order, $field);
    }
    ...
    public static function userRemove($role_id, $user_ids = [])
    {
        ...
        MerchantUserCache::del($user_ids);
    ...
    public static function menu_ids($role_id, $where = [])
    {
        ...
        $RoleModel = new MerchantRoleModel();
