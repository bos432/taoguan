 ...
 * 用户管理验证器
 */
class InspectionUserValidate extends Validate
{
    // 验证规则
    protected $rule = [
        'ids'      => ['require', 'array'],
    ...
    protected $scene = [
        ...
        'edit'     => ['ins_user_id', 'number', 'nickname', 'username', 'phone', 'email'],
        'dele'     => ['ids'],
    ...
    {
        return $this->only(['ins_user_id', 'number', 'nickname', 'username', 'phone', 'email'])
            ->append('ins_user_id', ['checkIsSuper']);
    }
    ...
    {
        $ids = $data['ids'] ?? [];
        if ($data['ins_user_id'] ?? 0) {
            $ids[] = $data['ins_user_id'];
        }

        foreach ($ids as $id) {
            $is_super = ins_user_is_super(ins_user_id());
            $ins_user_id  = ins_user_is_super($id);
            if (!$is_super && $ins_user_id) {
    ...
    {
         $info = InspectionUserAttributesModel::where('ins_user_id', 'in', $data['ids'])->where('role_id', '>', 0)->find();
         if ($info) {
             return '用户存在角色，请在[角色]中解除后再删除：' . $info['ins_user_id'];
         }
        return true;
    }
}
