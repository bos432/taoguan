...
class MobileAdminAccessService
{
    ...
    public static function decorateMemberInfo(array $member = []): array
    {
        ...
        $member['is_mobile_admin_enabled'] = !empty($access['has_any_permission']) ? 1 : 0;
        $member['is_admin_super'] = !empty($access['has_any_permission']) ? 1 : 0;
    ...
    private static function isMerchantSuperMember(array $member = []): bool
    {
        ...
        try {
            $query = MerchantModel::where('is_delete', 0)->where('member_is_super', 1);
