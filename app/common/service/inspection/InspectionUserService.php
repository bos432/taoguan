...
use app\common\service\inspection\InspectionUserTokenService;
...
class InspectionUserService
{
    ...
    public static function list($where = [], $page = 1, $limit = 10,  $order = [], $field = '')
    {
        ...
        if (ins_user_hide_where()) {
            $where[] = ins_user_hide_where('m.ins_user_id');
        }

        $model = $model->alias('m');
        foreach ($where as $wk => $wv) {
            if ($wv[0] == 'role_ids' && is_array($wv[2])) {
                $model = $model->join('inspection_user_attributes r', 'm.ins_user_id=r.ins_user_id')->where('r.role_id', $wv[1], $wv[2]);
                unset($where[$wk]);
            }
        }
    ...
    {
        $info = InspectionUserCache::get($id);
        if (empty($info)) {
            $model = new InspectionUserModel();
            ...
            $info = $info
                ->append(['avatar_url', 'role_ids','ins_title'])
                ->hidden(['avatar', 'roles','inspection'])
                ->toArray();
            ...
            if (user_is_super($id)) {
                ...
                $menu      = $InspectionMenuModel->field($MenuPk . ',menu_url')->where(where_disdel())->select()->toArray();
                $menu_ids  = array_column($menu, 'menu_id');
                $menu_urls = array_filter(array_column($menu, 'menu_url'));
            } else {
                $role_menu_ids   = InspectionRoleService::menu_ids($info['role_ids'], where_disdel());
                $unauth_menu_ids = InspectionMenuService::unauthList('id');
                $menu_ids        = array_merge($role_menu_ids, $unauth_menu_ids);
                $menu_urls       = $InspectionMenuModel->where('menu_id', 'in', $menu_ids)->where(where_disdel())->column('menu_url');
            ...
            $info['menus']    = InspectionMenuService::menus($menu_ids);
            $info['menu_ids'] = $menu_ids;

            InspectionUserCache::set($id, $info);
        }
        ...
        if ($role) {
            ...
            $menu_list = InspectionMenuService::list('list', [where_delete()], [], 'menu_id,menu_pid,menu_name,menu_url,is_unlogin,is_unauth');
    ...
    public static function edit($ids, $param = [])
    {
        ...
        try {
            ...
            // 修改
            $model->where('is_admin','<>',1)->where('ins_id',$ins_id)->where($pk, 'in', $ids)->update($param);
            if (var_isset($param, ['role_ids'])) {
                foreach ($ids as $id) {
                    $info = $model->where('is_admin','<>',1)->where('ins_id',$ins_id)->find($id);
                    // 修改角色
                    if (isset($param['role_ids'])) {
    ...
    public static function dele($ids, $real = false)
    {
        ...
        try {
            ...
            if ($real) {
                ...
                $model->where('ins_id',$ins_id)->where($pk, 'in', $ids)->delete();
            } else {
                $update = delete_update();
                $model->where('ins_id',$ins_id)->where($pk, 'in', $ids)->update($update);
            }
            // 提交事务
            $model->commit();
        } catch (\Exception $e) {
    ...
    public static function login($param)
    {
        ...
        $field = $pk . ',ins_id,password,is_disable,login_num';
        ...
        /********************判断检测机构是否被禁用或删除********************************/
        $mer = InspectionModel::where('id',$user['ins_id'])->where('is_delete','<>',1)->field('id,is_disable,is_delete')->find();
        if (empty($mer)) {
            exception('账号信息不存在');
        }
        if($mer['is_disable'] ==1){
        ...
        $user_log['ins_id'] = $user['ins_id'];
        InspectionUserLogService::add($user_log, SettingService::LOG_TYPE_LOGIN);

        InspectionUserCache::del($user_id);
    ...
    {
        $data = [];
        $setting = InspectionSettingService::info();
        $token_name = $setting['token_name'];
        $data[$token_name] = self::token($user);
        $fields = ['user_id', 'nickname', 'username','ins_title'];
    ...
    {
        return InspectionUserTokenService::create($user);
    }
